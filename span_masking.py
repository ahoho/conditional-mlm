...
github.com/facebookresearch/SpanBERT/blob/master/pretraining/fairseq/data/masking.py
"""
...
class ParagraphInfo(object):
    def __init__(self, tokenizer):
        ...
        if isinstance(tokenizer, BertTokenizer):
            self.start_words = {
                idx: not tok.startswith("##")
                for tok, idx in tokenizer.get_vocab().items()
            }
        else:
            raise NotImplementedError("Only support for BERT wordpiece tokenization")

    def get_word_piece_map(self, sentence):
        return [self.start_words[i] for i in sentence]
    ...
    for start, end in spans:
        ...
        for i in range(start, end + 1):
            ...
            if rand < 0.8:
                ...
                # sample random token according to input distribution
                sentence[i] = np.random.choice(tokens)
    pair_targets = None # pad_to_len(pair_targets, pad, pad_len + 2)
